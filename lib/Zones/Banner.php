...
class MASE_Zones_Banner {
    public static function init() {
        if(is_admin()) add_action('wp_ajax_mase_banner_zone', array('MASE_Zones_Banner', 'wp_ajax_mase_banner_zone'));
        if(is_admin()) add_action('wp_ajax_mase_banner_zone_save', array('MASE_Zones_Banner', 'wp_ajax_mase_banner_zone_save'));
    }

    public static function wp_ajax_mase_banner_zone() {
        ...
        $selected_size = isset($_GET['size']) ? sanitize_text_field($_GET['size']) : false;
        $widget_data = get_option('widget_'.strtolower('MASE_Banner_Widget'));
        $selected_widget = isset($widget_data[$widget_number]) ? $widget_data[$widget_number] : false;
        $zone_identifier = MASE_PREFIX.'banner_zone_ads_'.$widget_number;
        $zone_ads = get_option($zone_identifier);

        if(empty($widget_number) || empty($selected_size) || empty($selected_widget) || empty($widget_id)) { echo "Could not find Widget settings"; die(); }

        $ads = MASE_Ads_Generic::GetAds(array('size' => $selected_size, 'post_types' => array('mase_banner_ads', 'mase_html_ads')));
        ...
        <form class="mase_zone_configurator_form">
            <input type="hidden" name="save_action" value="mase_banner_zone_save" />
            <input type="hidden" name="widget_number" value="<?php echo $widget_number; ?>" />
            <input type="hidden" name="selected_size" value="<?php echo $selected_size; ?>" />
        ...
        <script type="text/javascript">
            jQuery(document).ready(function() {
                ...
                var table = jQuery('#mase_zone_configurator_tbl').MaseTable(datatable_options);
    ...
    public static function wp_ajax_mase_banner_zone_save() {
        if(!isset($_REQUEST['widget_number'])) { die(); }
        $zone_identifier = MASE_PREFIX.'banner_zone_ads_'.(int)$_REQUEST['widget_number'];
