...
class MASE_Zones_Menu {
    public static function init() {
        if(is_admin()) add_action('wp_ajax_mase_menu_zone', array('MASE_Zones_Menu', 'wp_ajax_mase_menu_zone'));
        if(is_admin()) add_action('wp_ajax_mase_menu_zone_save', array('MASE_Zones_Menu', 'wp_ajax_mase_menu_zone_save'));
    }

    public static function wp_ajax_mase_menu_zone() {
        $menu_item_id = isset($_GET['menu_item_id']) ? (int)$_GET['menu_item_id'] : false;
        $zone_identifier = MASE_PREFIX.'menu_zone_ads_'.$menu_item_id;
        $zone_ads = get_option($zone_identifier);

        if(empty($menu_item_id)) { echo "Could not find Widget settings"; die(); }
        ...
        <form class="mase_zone_configurator_form">
            <input type="hidden" name="save_action" value="mase_menu_zone_save" />
    ...
    public static function wp_ajax_mase_menu_zone_save() {
        if(!isset($_REQUEST['menu_item_id'])) { die(); }
        $zone_identifier = MASE_PREFIX.'menu_zone_ads_'.(int)$_REQUEST['menu_item_id'];
